...
node_expected_version="v14.15"
node_version=`nodejs -v`
if [[ $node_version == $node_expected_version* ]]; then
    echo "!  Node.js $node_version installed "
else
    echo "ERROR: Incorrect Node.js version - $node_version (expected $node_expected_version.x)"
fi
npm_expected_version="6.14"
npm_version=`npm -v`
if [[ $npm_version == $npm_expected_version* ]]; then
    echo "!  NPM v$npm_version installed"
else
    echo "ERROR: Incorrect NPM version - v$npm_version (expected v$npm_expected_version.x)"
fi
